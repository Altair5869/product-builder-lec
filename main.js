...
const desserts = {
  ...
  icecream: {
    name: "젤라또 아이스크림",
    icon: "🍨",
    description: "이탈리아 정통 방식으로 만든 진하고 부드러운 아이스크림이에요. 우유 함량이 높아 더 크리미하고 풍부한 맛을 자랑해요.",
    tags: ["#시원함", "#크리미", "#여름필수"],
    tip: "피스타치오, 헤이즐넛 맛을 추천해요! 고소함이 일품이에요."
  },
  croffle: {
    name: "크로플",
    icon: "🧇",
  ...
  brownie: {
    name: "초코 브라우니",
    icon: "🍫",
    description: "진한 초콜릿의 풍미가 가득한 촉촉한 브라우니예요. 호두나 아몬드를 넣으면 고소함까지 더해져요.",
    tags: ["#초콜릿", "#진한맛", "#촉촉함"],
    tip: "전자레인지에 10초만 돌리면 갓 구운 듯한 따뜻함을 느낄 수 있어요!"
  },
  fruitTart: {
    name: "생과일 타르트",
    icon: "🥧",
    description: "바삭한 타르트지 위에 부드러운 커스터드 크림과 신선한 과일이 올라간 상큼한 디저트예요.",
    tags: ["#상큼함", "#과일가득", "#비주얼"],
    tip: "딸기, 블루베리, 망고 등 제철 과일이 올라간 타르트를 선택해보세요!"
  },
  crepe: {
    name: "딸기 크레페",
    icon: "🫓",
    description: "얇고 부드러운 크레페에 신선한 딸기와 생크림이 듬뿍! 달콤하고 상큼한 맛의 조화가 환상이에요.",
    tags: ["#프렌치", "#딸기", "#생크림"],
    tip: "누텔라를 추가하면 더 달콤한 맛을 즐길 수 있어요!"
  },
  donut: {
    name: "글레이즈드 도넛",
    icon: "🍩",
...
function calculateResult() {
  ...
  if (temp === 'cold' && texture === 'creamy') {
    return desserts.icecream;
  }
  ...
  if (taste === 'bitter' && mood === 'stressed') {
    return desserts.brownie;
  }
  if (taste === 'fresh' && texture === 'crunchy') {
    return desserts.fruitTart;
  }
  if (taste === 'fresh' && texture === 'soft') {
    return desserts.crepe;
  }
  ...
  if (mood === 'tired') return desserts.brownie;
  if (mood === 'stressed') return desserts.cheesecake;
  if (mood === 'excited') return desserts.crepe;
